...
def extract_qr_from_video(video_path: str, sample_rate: int = 0,
    ...
    Args:
        ...
        use_mnn: When True, use MNN-accelerated detection with
            ...
            preserves the existing behaviour.  ``use_mnn`` and
            ``detect_isolation`` are independent: MNN routes through
            the in-process :class:`DetectorRouter` and does not go
            through the subprocess sandbox, whereas the OpenCV
            fallback still honours ``detect_isolation``.
        detect_isolation: ``'on'`` (default) runs QR detection in a pool
            ...
            camera-captured inputs).  Only applies when ``use_mnn`` is
            False or when the MNN router falls back to OpenCV.
