...
- **M2 完成**：CPU 正式版打包落地。模型文件从 `dev/` 迁移到 `src/qrstream/detector/models/` 作为 package data 分发，支持 `QRSTREAM_MNN_MODEL_DIR` 环境变量覆盖，自动 fallback 到 dev 路径。`QRDetector` 基类新增 `detect_batch` 默认实现（M5 预留抽象），`DetectorRouter` 透传。改善 MNN 不可用 / 模型缺失时的用户友好错误提示。`Containerfile.m2` 全量回归 291 fast + 4 slow 全绿。
...
- ✅ `MNNQrDetector` 继承默认 `detect_batch`（M5 再覆写为真正的 batch 推理）
